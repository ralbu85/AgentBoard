...
%%EOF"""
    ...
    async with async_playwright() as p:
        ...
        # ── 3. Directory listing ──
        print("\n3. Directory listing")
        entries = await page.query_selector_all(".tree-row")
        if len(entries) > 0:
            ok(f"{len(entries)} entries displayed")
            # Check for expected files
            entry_names = await page.evaluate("""() =>
                [...document.querySelectorAll('.tree-row .file-name')].map(e => e.textContent)
            """)
            ...
            dir_entries = await page.query_selector_all(".tree-row.is-dir")
            file_entries_el = await page.query_selector_all(".tree-row.is-file")
        ...
        subdir_entry = await page.query_selector(".tree-row.is-dir")
        if subdir_entry:
