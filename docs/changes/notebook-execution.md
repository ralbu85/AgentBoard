...
선택기에서 이름·경로를 검색하고 **선택 및 연결**을 누르면 바로 연결된다. 연결된 커널이 있는 경우 다른 환경 선택 시 변수 초기화를 확인받은 뒤 전환한다. 선택한 환경은 복구 상태에 저장되며 커널 상태와 목록에 이름·경로를 표시한다. 서버 재시작이 필요한 배포 시 기존 연결 커널이 있으면 변수 손실을 피하기 위해 사용자 확인 후 반영한다.
